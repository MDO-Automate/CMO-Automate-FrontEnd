...
export class CirculationComponent implements OnInit {
  ...
  createCirculation(circulation: Circulation) {
    this.circulationsService.create(circulation)
      .subscribe(
        this.handlerResponseCrud('guardado')
      )
  }
  ...
  updateCirculation(circulation: Circulation) {
    ...
    this.circulationsService.update(id || 0, circulation)
      .subscribe(
        this.handlerResponseCrud('actualizado')
      )
  }

  deleteCirculation(circulation: Circulation) {
    this.circulationsService.delete(circulation?.id || 0)
      .subscribe(this.handlerResponseCrud('eliminado'))
  }

  assignCirculation(data: CirculationResponse[]) {
    this.circulations = data
    this.circulationsTable = this.circulations.map(({ id, circulacion, linea }) => {
